...
function getRuntimeKakaoKey(): string {
  const urlKey = new URLSearchParams(window.location.search).get("key") || "";
  const storedKey = window.localStorage.getItem("KAKAO_MAP_KEY") || "";
  const runtimeKey = window.KAKAO_MAP_KEY || "";
  const key = urlKey || storedKey || runtimeKey;
  if (urlKey) {
    window.localStorage.setItem("KAKAO_MAP_KEY", urlKey);
  }
  return key;
}
...
function loadKakaoSDK(): Promise<void> {
  const kakaoKey = getRuntimeKakaoKey();
  if (!kakaoKey) {
