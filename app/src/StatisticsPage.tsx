...
} from "./statisticsPreviewData";
import { CITY_NAME, CITY_SHORT_NAME } from "./config/regionConfig";
...
function kpiCard(title: string, value: string, helper: string) {
  return { title, value, helper };
}
  ...
  const districtChartData = useMemo(
    () =>
      data.districts.map((district) => ({
        ...
        vulnerabilityScore: district.case1Count * 1.4 + district.case2Count + district.avgNearestParkDistanceM / 220,
      })),
    [data.districts]
  );

  const summaryCards = [
    kpiCard("전체 학교", `${formatNumber(data.summary.schoolCount)}개교`, `${formatNumber(data.summary.districtCount)}개 구·군 비교`),
    kpiCard("즉시 개선 대상", `${formatNumber(data.summary.urgentSupportCount)}개교`, "공원 접근 불가 중심"),
    kpiCard("우선 검토 대상", `${formatNumber(data.summary.priorityReviewCount)}개교`, "공원 접근 가능 · 녹지 부족 포함"),
    kpiCard("2029 잠재 수요", `${formatNumber(data.summary.totalPotentialDemand2029)}명`, "시 전체 잠재 수요 합계"),
  ];
    ...
    <div className="mx-auto flex max-w-[1380px] flex-col gap-8 px-4 py-8 lg:px-8">
      <section className="space-y-5 rounded-[32px] border border-slate-200 bg-white p-7 shadow-sm">
        <div className="space-y-2">
          ...
          <h1 className="text-4xl font-black tracking-tight text-slate-950 lg:text-5xl">{data.cityName} 학교 환경 통계 리포트</h1>
          <p className="max-w-3xl text-base leading-7 text-slate-600">
            시 전체 우선 지원 대상과 각 구·군별 대표 학교를 한 화면에서 비교할 수 있도록 구성한 통계 프리뷰입니다.
            먼저 시 전체 압력을 보고, 이어서 구별 우선 지원 학교 Top 5와 최우수 학교 1개를 내려보는 흐름으로 설계했습니다.
          </p>
        </div>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
          <SectionTitle
            ...
            title="구별 우선 지원 압력"
            description="즉시 개선 대상과 우선 검토 대상을 함께 보고, 어디에 생활권 취약성이 집중되는지 빠르게 비교합니다."
          />
          <div className="mt-5 h-[360px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={districtChartData} layout="vertical" margin={{ left: 8, right: 10, top: 8, bottom: 8 }}>
                ...
                <YAxis dataKey="districtName" type="category" width={62} tick={{ fontSize: 12, fill: "#334155", fontWeight: 600 }} />
                <Tooltip
                  cursor={{ fill: "#f8fafc" }}
                  formatter={(value: number, name: string) => {
                    if (name === "urgentSupportCount") return [`${formatNumber(value)}개교`, "즉시 개선 대상"];
                    if (name === "priorityReviewCount") return [`${formatNumber(value)}개교`, "우선 검토 대상"];
        ...
        <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
          <SectionTitle
            eyebrow="City Best"
            title="시 전체 최우수 학교"
            description="생활환경 기준학교 역할을 하는 학교 1개를 따로 떼어 보여줍니다."
          />
          <div className="mt-5">
            <BestSchoolCard school={data.cityBestSchool} label={`${CITY_SHORT_NAME} 최우수 학교`} />
          </div>
        </div>
      </section>
      ...
      <section className="space-y-5 rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
        <SectionTitle
          ...
          title="구별 상세 통계"
          description="구를 선택하면 해당 구의 전체 지표와 우선 지원 학교 Top 5, 그리고 구 최우수 학교 1개를 바로 확인할 수 있습니다."
        />
        <div className="flex flex-wrap gap-2">
          {data.districts.map((district) => {
            const isActive = district.districtName === selectedDistrict?.districtName;
            return (
              <button
            ...
            <div className="space-y-5">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <SummaryCard
                  ...
                  helper={`우선 검토 대상 ${formatNumber(selectedDistrict.priorityReviewCount)}개교`}
                />
              </div>
      ...
      <section className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
          <SectionTitle
            ...
            description="시 단위에서 즉시 지원 검토가 가장 필요한 학교를 잠재 수요와 취약성 흐름 기준으로 빠르게 읽도록 정리했습니다."
          />
        ...
        <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
          <SectionTitle
            eyebrow="City Mix"
            title="구별 2029 잠재 수요"
            description="구 단위 총 잠재 수요 규모를 막대로 비교해 어디가 우선 검토 대상인지 빠르게 볼 수 있습니다."
          />
                ...
                <Bar dataKey="totalPotentialDemand2029" radius={[8, 8, 0, 0]}>
                  {districtChartData.map((entry) => (
                    <Cell
                      ...
                      fill={entry.districtName === selectedDistrict?.districtName ? "#0f172a" : "#94a3b8"}
                    />
                  ))}
                </Bar>
              </BarChart>
