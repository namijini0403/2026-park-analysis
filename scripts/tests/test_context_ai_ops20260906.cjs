...
async function main() {
  ...
  console.log("[2] 키 없는 폴백: 유흥 관측 질문 (인천신광초 B000002950, 관측 1건·최근접 434m)");
  const night = await ask(identified(
    "이 학교 주변 유흥주점 인허가 관측이 있나요?",
    { school_id: "B000002950", school_name: "인천신광초등학교" },
  ));
  ok("answerable=true + 관측 1건 하한 표현", night.json.answerable === true && JSON.stringify(night.json).includes("최소 관측 1건"));
  ok("전수 아님(부분 커버리지) 명시", JSON.stringify(night.json).includes("전수 아님"));
  ok("최근접 관측 거리 포함", JSON.stringify(night.json).includes("434"));
  ...
  ok("서버 관측치 1건 사용", spoofText.includes("최소 관측 1건"));
