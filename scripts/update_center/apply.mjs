...
import { verifyStaging } from "./candidate.mjs";
...
export async function applyStagedCandidate({ entry, stagingId, store, eventId, actor, confirm = false, log = () => {} }) {
  ...
  return { version: versionName, versionId: version.id, versionDir, manifest, rebuild, contentHash, written };
}
 ...
 */
export async function rollbackVersionDir({ versionName, entry, store, actor, log = () => {} }) {
  const manifest = readVersionManifest(versionName);
  if (!manifest) throw new Error(`버전 매니페스트를 찾을 수 없습니다: ${versionName}`);
  ...
  return { version: versionName, restoredFiles: restored, removedFiles: removed, activeVersion: previousVersion, rebuild };
}
