...
export function refreshCoverage(){
  ...
  const items=sources.map(s=>({dataset:s.dataset,file:s.local_file,provider:s.provider,source_url:s.source_url,check:s.check?.type||'manual',
    mode:s.auto_apply&&!s.never_auto_apply&&!s.rebuild_command?'validated_auto_apply':s.check?.type==='json_api'?'collect_review_apply':s.check?.type==='file_head'?'header_signal_only':s.check?.type==='page_notice'?'notice_signal_only':s.check?.type==='school_zones'?'collect_review_apply':'manual',
  ...
  return {generated_at:new Date().toISOString(),counts,sources:items,unregistered_files:unregistered,
    notes:['미등록 파일에는 다른 원자료에서 만들어지는 파생 산출물도 포함됩니다. 모두 독립 수집 대상이라는 뜻은 아닙니다.','file_head는 HTTP 헤더, page_notice는 게시글·첨부 참조 변화만 확인하며 첨부파일 내용 갱신이나 새 게시물 발견을 보장하지 않습니다.','자동 반영은 출처별 명시적 허용과 품질 검사를 모두 통과해야 합니다.','기존 모델/생활권/정책 결과는 전체 파생 재분석이 연결되어야 갱신 완료라고 할 수 있습니다.']};
}
