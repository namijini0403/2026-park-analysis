...
입력 (repo 자체 포함, 원 출처는 outputs/source_research_20260906/ + PROVENANCE 참조)
  ...
  data/context_sources/incheon_nightlife_geocoded.csv 유흥·단란주점 인허가 1,534 (좌표 1,222)
  data/context_sources/construction_geocoded_exact.csv          연수구 착공 행정기록, 좌표 추정 15
  data/context_sources/construction_yeonsu_exact_unmatched.csv  연수구 착공 행정기록, 미매핑 59
...
DESIGNATION_FILES = [
    "school_designations_2026.csv",
    "school_ai_focus_2026.csv",
    "school_digital_tutor_2025.csv",
]

NIGHTLIFE_FILE = "incheon_nightlife_geocoded.csv"
CONSTRUCTION_FILES = ["construction_geocoded_exact.csv", "construction_yeonsu_exact_unmatched.csv"]
CONSTRUCTION_COVERED_GUS = ["연수구"]
...
def build(sources_dir: Path = SOURCES_DIR,
    ...
    for school in schools:
        ...
        if construction_available and school["gu"] in CONSTRUCTION_COVERED_GUS:
            ...
            summary["construction"] = {"status": "unknown", "observed_count": None, "total_count": None,
                                       "label_ko": "자료 수집 전" if not construction_available else "해당 구 자료 미수집 (연수구만 수집)"}
    ...
    if construction is None:
        ...
        layers["construction_records"] = {
            ...
            "status_label_ko": "부분 수집(연수구, 좌표 일부)" if construction_available else "좌표 확보 레코드 없음",
            "label_ko": "공사장(착공신고) 행정기록",
            ...
            "coverage_basis_ko": "학교 소재지 주소의 구 기준. 그 외 구 학교는 미수집(unknown)으로 표시.",
            "coverage_note_ko": "연수구 74건 중 15건만 동일 지번 전체주소 정확일치로 좌표 추정(소상공인 상가정보 2026-06-30 기준). 나머지 59건은 미매핑 상태로 보존. 계양·미추홀 원자료는 좌표 미확보로 미반영.",
            "coordinate_note_ko": "좌표는 상가 주소 기반 추정 위치이며 실제 공사현장 좌표가 아님.",
            "distance_basis_ko": "직선거리 기준 (도보 경로 아님)",
            "sources": [
                ...
                {"url": "https://www.data.go.kr/data/15083033/fileData.do", "title": "소상공인시장진흥공단 상가(상권)정보 (좌표 추정 기준)", "published_date": None, "source_as_of": "2026-06-30", "retrieved_at": as_of},
            ],
