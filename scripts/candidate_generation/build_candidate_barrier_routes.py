...
SCHOOLS_PATH = BASE / "data/processed" / "schools.csv"
SCHOOL_PRIORITY_PATH = BASE / "data/processed" / "school_priority.csv"
CANDIDATES_PATH = BASE / "data/processed" / "candidate_grid_final.geojson"
OUTPUT_JSON_PATH = BASE / "data/processed" / "candidate_barrier_routes_by_school.json"
OUTPUT_CSV_PATH = BASE / "output" / "candidate_barrier_routes_by_school.csv"
MIN_CANDIDATES_PER_CASE123_SCHOOL = 6
SUPPLEMENTAL_CANDIDATE_MAX_DISTANCE_M = 600
...
def load_accident_hotspots() -> list[dict[str, Any]]:
    accident_path = BASE / "data/raw" / ACCIDENT_DATA_FILENAME
    ...
    for school_name, school_info in schools_by_name.items():
        ...
        for candidate in candidate_rows:
            ...
            if grid_id in existing_ids:
                continue
            straight_distance = distance_in_meters(
                school_info["lat"],
                school_info["lng"],
                candidate["lat"],
                candidate["lng"],
            )
            if straight_distance > SUPPLEMENTAL_CANDIDATE_MAX_DISTANCE_M:
                continue
            ranked_candidates.append((straight_distance, candidate["candidate_rank"], grid_id))
