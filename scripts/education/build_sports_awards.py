...
NOTE = '대한체육회 2024·2025 전국체육대회 인천 메달 명세에서 현재 원장 학교명과 정확히 연결한 관측입니다. 학교·종목·종별·세부종목·등급·일자별로 묶었습니다. 선수별 행 수와 관측 수는 메달 수가 아닙니다. 단체전·복식·개인전 중복 집계와 보도자료 중복을 피하기 위해 총 메달 수를 산출하지 않습니다. 선수 이름·선수 식별자는 저장하지 않습니다. 기록 없음은 수상 없음이 아닙니다.'
...
def main():
    ...
    for snapshot in snapshots:
        for record in snapshot['records']:
            matches=[r for r in registry if r['학교명']==record['school_name']]
            if len(matches)!=1:
                if '학교' in record['school_name']: held.append({'year':snapshot['year'],'school_name':record['school_name'],'reason':'No unique exact current registry match'})
                continue
            sid=matches[0]['학교ID']
            schools.setdefault(sid,[]).append({**record,'year':snapshot['year'],'event':snapshot['event'],'result':record['medal']+'메달 관측','source_url':snapshot['url'],'participant_scope':'school_event_medal_observation','result_basis':'학교별 결과 관측이며 메달 총수·선수 수 아님'})
