...
- 지표 프로필: 학교를 선택하면 지도 아래에 영역 9개(지정사업·공원·도서·학원·안전·도보권/학구도·학생추세·학생교원·주변개발)의 절대값과 상대 위치가 뜬다. 비교는 같은 학교급 안에서만 하고, 레이더 기준은 인천 전체 / 군·구를 토글한다. 백분위는 값이 큰 쪽 기준 고정 정의이며(챗봇의 질문 방향 백분위와 다른 숫자다), 표본 10개교 미만 군·구는 백분위를 숨기고 순위만 쓴다. 레이더는 영역을 합산하지 않고 영역 대표 지표 1개씩을 축으로 삼으며, 미확보 축은 0이 아니라 끊어진 축으로 그린다. `npm run test:profile`
- 영역별 통계: `05 영역별 통계` 탭에서 영역·학교급을 골라 분포·군구별 비교·커버리지를 본다. 강화·옹진 도서지역은 따로 센다.
- 학교 요약 정리: 모든 학교에서 문구가 같던 ‘검토할 선택지’와 패널에 중복되던 ‘확인된 사실’ 카드를 없앴다. 모든 학교에 붙던 고정 안내는 접힌 ‘분석의 한계’로 옮기고 학교별로 달라지는 조건만 남겼다.
