...
import { CITY_AVG, CITY_NAME } from "./config/regionConfig";
import { MANUAL_BARRIER_OVERRIDES } from "./config/manualQAOverrides";
export type { ManualBarrierOverride } from "./config/manualQAOverrides";
...
export function mapSchoolRowToReportProps(
  ...
  const districtName = gu ? `${CITY_NAME} ${gu}` : CITY_NAME;
