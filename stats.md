...
## 0. 보행부담 동반형 태그 재정의 검증

작성일: 2026-05-26

최종 앱 기준:

> 보행부담 동반형 별도 태그 = 최근접 활동규모 공원 도달 경로에 간선급 도로 횡단 1회 이상 또는 대형 교차로 통과가 확인되는 학교
...
| 정책분류 대상 학교 수 | 242개교 |
| 별도 보행부담 동반형 태그 | 15개교 |
| 간선급 도로 횡단 1회 이상 | 15개교 |
| 대형 교차로 통과 | 0개교 |
| 기본 접근유형이 보행부담 동반형인 학교 | 5개교 |
| 근접 공원-녹지환경 불균형형에 함께 걸린 보행부담 태그 | 10개교 |
| 사고위험·우회율만으로 보행부담 라벨이 남은 학교 | 0개교 |

앱 반영 방식:

- `access_condition_type`은 배타적 기본 접근유형으로 유지한다.
- 보행부담 동반형은 `functional_access_physical_barrier_flag` 별도 태그로 관리한다.
- 앱의 `보행부담 동반` 필터는 `access_condition_type`이 아니라 `functional_access_physical_barrier_flag` 기준으로 15개교를 표시한다.
...
| candidate_grid_current_vs_2031 | 후보지 현재 기준 아동수 | 후보지 2031 예측 수혜값 | 0.147 | 85 | 15/85 | 0.097 | K=85(5.5%): 현재 인구 분포와 미래 수요 후보 순위는 크게 달라짐 |
| candidate_grid_current_vs_2031 | 후보지 현재 기준 아동수 | 후보지 2031 예측 수혜값 | 0.147 | 155 | 57/155 | 0.225 | K=155(10%, 주력): 상위 10%에서도 현재·미래 순위 불일치 |
...
| K | 전체 대비 | n_sim | 평균 Jaccard | 중앙 Jaccard | 평균 유지율 | 중앙 유지율 | 평균 순위 변동 | Jaccard p10~p90 |
|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 50 | 3.2% | 1,000 | 0.953 | 0.961 | 97.57% | 98.00% | 2.08 | 0.923~1.000 |
| 85 | 5.5% (구 iso_green_ratio 기준 Case1+2=85) | 1,000 | 0.883 | 0.889 | 93.75% | 94.12% | 3.41 | 0.848~0.910 |
| **155** | **10% (주력)** | 1,000 | **0.923** | 0.925 | **96.00%** | 96.13% | 5.77 | 0.902~0.950 |

PPT 본문용 숫자 (K=155 기준):

| 항목 | 값 | 비고 |
|---|---:|---|
| Backtest MAE | 1,041.1명 | 구별 총량 1-step backtest |
| Backtest WAPE | 3.66% | 1-step backtest 전체 (origin 2020~2024) |
| Backtest WAPE (multi-step horizon=1) | 3.64% | origin 2021+, multi-step 방식 |
| 후보지 Top-K 유지율, K=155 (상위 10%) | 96.00% | |
| 후보지 Jaccard, K=155 | 평균 0.923 / 중앙값 0.925 | |
| Perturbation 횟수 | 1,000회 | |

WAPE 주의: 3.66%(1-step 전체)와 3.64%(multi-step horizon=1)는 방법론이 다르다. PPT에는 3.66%가 더 명확하고 방어하기 쉽다. 3.64%를 쓰면 "multi-step 방식 기준"을 명시해야 한다. 두 경우 모두 Baseline 2(구별 직전 증감률)가 2.63%로 더 낮다.
...
> 구별 총량 backtest에서 Proposed WAPE는 3.66%였고, 예측 잔차를 반영해 후보지 수요를 1,000회 흔들어도 상위 10%(K=155) 후보의 평균 유지율은 96.0%였다.
...
| Top-K | 전체 대비 | Proposed vs Baseline2-equivalent 겹침 | Jaccard |
|---:|---:|---:|---:|
| 50 | 3.2% | 7 / 50 | 0.075 |
| 85 | 5.5% | 15 / 85 | 0.097 |
| **155** | **10% (주력)** | **34 / 155** | **0.123** |

K=155 안정성 직접 비교 (2026-05-27 재검증, random seed=42, N=1,000회):

| 모델 | 잔차 출처 | K=155 평균 Jaccard | 평균 유지율 | 평균 순위변동 |
|---|---|---:|---:|---:|
| **Proposed** | Prophet residual | **0.923** | **96.01%** | 5.81 |
| **Baseline2** | baseline_gu_growth residual | **0.934** | **96.56%** | 5.97 |

Proposed vs Baseline2 top-155 겹침: **24/155, Jaccard 0.084**

한 줄 해석:

> Baseline2가 perturbation 안정성은 근소하게 높지만(96.6% vs 96.0%), 두 모델이 선택하는 상위 10% 후보는 24/155(Jaccard 0.084)만 겹쳐 — 추천 방향이 근본적으로 다르다. Track 2의 차별점은 안정성 우위가 아니라 다른 후보군을 도출한다는 데 있다.

안전한 해석 (PPT):

> Baseline2(단순 현재 아동수 외삽)도 perturbation 안정성은 96.6%로 Proposed(96.0%)와 비슷하다. 두 모델의 차이는 안정성이 아니라 **선택하는 후보**에 있다: 상위 10%(155개) 중 24개만 겹치고(Jaccard 0.084), Baseline2는 현재 아동수가 많은 곳을, Proposed는 미래 수요 증가와 학교 접근성을 반영한 곳을 고른다.

PPT 후보 차별성 문구 (K=155 기준):

> 단순 현재 아동 인구 기준 상위 10%와 수혜 예측 기준 상위 10%는 24/155(Jaccard 0.084)만 겹친다. 현재 인구가 많은 곳이 아니라 미래 수요 변화와 학교 접근성을 반영한 후보를 선택하기 위해 예측 모델이 필요한 근거다.

### 9-9. Proposed vs Baseline2-equivalent Top-85 및 Top-155 정책 타겟 특성 비교
...
**Top-155 (상위 10%) 특성 비교 — 2026-05-27 추가**

| 그룹 | n | 현재 아동수 평균 | 2031 격자 예측 평균 | 변화율 |
|---|---:|---:|---:|---:|
| Proposed Top-155 | 155 | 105.6명 | 173.7명 | +64.6% |
| Baseline2-equivalent Top-155 | 155 | 280.3명 | 115.1명 | -58.9% |

Proposed Top-155 case_type 분포: Case1=15, Case2=55, Case3=44, Case4=41

한 줄 요약 (Top-155 기준):

> Proposed 상위 10%(155개)는 현재 아동수 평균 105.6명이지만 2031 예측은 173.7명(+64.6%)이고, Baseline2-eq는 현재 280.3명에서 115.1명(-58.9%)으로 감소한다. 겹침은 34/155(Jaccard 0.123)에 불과해, 두 방식이 선택하는 후보군 성격이 근본적으로 다르다.
...
검증일: 2026-05-26
재현 스크립트: `scripts/audit_case_threshold_sensitivity.py`
산출물:

- `outputs/case_threshold_sensitivity/case_threshold_sensitivity.csv`
- `outputs/case_threshold_sensitivity/case_threshold_sensitivity_heatmap.png`
- `outputs/case_threshold_sensitivity/case_threshold_sensitivity_bar.png`

대상: `is_separate_bundle_tag == 0`, 242교. Case 1 조건 고정(nearest_park_dist_m ≥ 500 AND iso_park_count == 0 AND display_green_ratio == 0), upper=5% 고정, lower 임계값 변화.

**lower 임계값 민감도 (upper=5% 고정)**

| lower | Case 1 | Case 2 | Case 3 | Case 4 | Case 1+2 | 현재 대비 Case 1+2 |
|---:|---:|---:|---:|---:|---:|---:|
| 0.5% | 17 | 42 | 104 | 79 | 59 | -22 |
| **1.0% (현재)** | **17** | **64** | **82** | **79** | **81** | **0** |
| 1.5% | 17 | 81 | 65 | 79 | 98 | +17 |
| 2.0% | 17 | 95 | 51 | 79 | 112 | +31 |
| 3.0% | 17 | 115 | 31 | 79 | 132 | +51 |

upper 임계값(3~10%)은 Case 2 수에 영향 없음 — Case 3/4 분할만 변화.

**lower 임계값 근방 분포 (접근 가능 225교)**

| 구간 | 학교 수 | 비고 |
|---|---:|---|
| 0% ~ 0.5% | 42교 | |
| 0.5% ~ 1.0% | 22교 | ← 1% 경계 바로 아래, lower 낮추면 Case 3 이동 |
| 1.0% ~ 1.5% | 17교 | ← 1% 경계 바로 위, lower 높이면 Case 2 편입 |
| 1.5% ~ 2.0% | 14교 | |
| 2.0% ~ 3.0% | 20교 | |
...
> 현재 녹지율 1% 미만 기준(Case 2)으로 64교가 분류된다. lower 임계값을 0.5%로 낮추면 42교, 2%로 올리면 95교로 변화해 경계 근방에 22~17교가 집중 분포하므로, 임계값 선택이 Case 2 규모에 민감하게 작용한다.
...
# n=1551 → k_values = [50, 85, 155]
```

- 입력 데이터: `robust_candidate_recommendations.csv` — **195개 학교(Case 1~4 전체)** × 후보지 = 1,551개 school::candidate 쌍
- K=85 = **구 `iso_green_ratio` 기준 Case 1+2 학교 수(17+68=85)** 와 일치 → 당시 "우선지원 대상 학교 수"를 K로 쓰려는 의도로 하드코딩된 것으로 추정
- 현재 `display_green_ratio` 기준 Case 1+2 = **81**로 변경되어 K=85는 더 이상 Case 1+2와 일치하지 않음
- K=85는 전체 1,551쌍의 **5.5%**; K=50은 **3.2%**; K=155는 **10%**

**K별 전체 대비 비율 정오표:**

| K | 전체(1,551) 대비 | 평균 유지율 | PPT에서 쓸 표현 |
|---:|---:|---:|---|
| 50 | **3.2%** | 97.57% | "상위 50개(약 3%)" |
| 85 | **5.5%** | 93.75% | 구 기준 잔재 — PPT 주력 사용 비권장 |
| 155 | **10%** | 96.00% | **"상위 10%(155개)"** ← 주력 사용 |
...
## 18. Case 임계값 부등호 표기 차이 (제출 PDF vs 실제 코드)

검증일: 2026-06-03

발표 시 구두 질의 대비용 메모. 제출 PDF(`반경너머_도달가능성으로`, 슬라이드 9)와 실제 분류 코드(`apply_case_system_20260411.py`)에서 **Case 2~4 녹지비율 경계의 부등호 포함/제외 표기가 서로 반대**다. 챗봇(RAG 해설)은 코드 기준과 일치하므로 정확하다.

| 구분 | Case 2 (우선검토) | Case 3 (모니터링) | Case 4 (유지·관리) |
|---|---|---|---|
| 제출 PDF 슬라이드 9 | 녹지비율 **≤ 1%** | **1% < x ≤ 5%** | **> 5%** |
| 실제 코드 / 챗봇 | 녹지비율 **< 1%** | **1% ≤ x < 5%** | **≥ 5%** |

코드 근거 (`apply_case_system_20260411.py`):
...
- 즉 코드 기준은 **Case 2 = 1% 미만**, **Case 4 = 5% 이상**, 그 사이가 Case 3이다.
- PDF는 경계에서 `≤ / >`를 써서 정확히 1.000% 또는 5.000%인 학교를 Case 2·3 쪽에 포함시키는 것처럼 표기됐다.
- **실제 영향 없음**: `display_green_ratio`는 연속 실수값이라 정확히 1.000%·5.000%에 걸리는 학교가 사실상 존재하지 않아 분류 결과는 동일하다.

### 구두 질의 대비 답변

> "코드·챗봇 기준은 **Case 2가 1% 미만, Case 4가 5% 이상**(미만/이상 기준)입니다. PDF 슬라이드의 부등호 표기와 포함·제외 방향이 한 칸 다르게 보일 수 있으나, 녹지비율이 정확히 1%·5%인 학교가 없어 실제 분류 결과는 같습니다. 챗봇 해설은 코드 기준으로 정확하게 답합니다."

제출본 PDF는 이미 제출되어 수정하지 않으며, 챗봇 근거 문서도 코드와 일치하므로 변경하지 않는다.
