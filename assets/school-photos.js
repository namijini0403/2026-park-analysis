...
(function(global){
 ...
 function mount(host,school){
  ...
  host.innerHTML='<h3>학교·주변 시설 사진</h3><p>Google 등록 사진에는 다른 장소나 오래된 사진이 포함될 수 있습니다. 사진이 현재 상태·안전·출입 가능 여부를 확인해 주지는 않습니다.</p>'
   +'<div class="photo-controls"><label>사진을 볼 장소<select aria-label="사진을 볼 장소"></select></label><button type="button">사진 찾아보기</button></div>'
   +'<p class="photo-auto-note">선택한 학교의 사진 후보를 자동으로 불러옵니다. 주변 시설은 장소를 고른 뒤 사진 찾아보기를 눌러 주세요.</p><p class="photo-coverage">주변 시설 목록을 확인하고 있습니다.</p><p class="photo-status" role="status" aria-live="polite"></p><div class="photo-results"></div>';
  ...
  let choices=[{...school,kind:'학교'}],revision=0,timer;
  const active=()=>host.isConnected&&host.querySelector('.photo-results')===results;
  const cancel=()=>{revision++;clearTimeout(timer);};
  mounts.set(host,cancel);
  ...
  select.onchange=()=>{cancel();results.replaceChildren();button.disabled=false;button.textContent='사진 찾아보기';status.textContent='선택한 장소의 사진을 찾아보세요.';if(select.value==='0')button.onclick();};
  button.onclick=async()=>{
   cancel();const ticket=revision,p=choices[Number(select.value)];button.disabled=true;button.textContent='사진 다시 찾기';results.replaceChildren();status.textContent=p.kind+' · '+p.name+'의 사진 후보를 불러오고 있습니다. 학교 분석은 계속 볼 수 있습니다.';
   try{
    await ready();if(ticket!==revision||!active())return;
    ...
    timer=setTimeout(()=>{if(ticket===revision&&active()){status.textContent='사진 응답이 지연되고 있습니다. 다시 시도해 주세요.';button.disabled=false;}},25000);
    const finish=message=>{if(ticket!==revision||!active())return;clearTimeout(timer);status.textContent=message;button.disabled=false;};
    function photoCard(place,title,className){
     const detail=document.createElement('section');detail.className=className;
     const heading=document.createElement('h4');heading.textContent=title;detail.append(heading);
     ...
     const request=document.createElement('gmp-place-details-place-request');request.place=place;
     card.addEventListener('gmp-error',()=>{heading.textContent='사진을 불러오지 못했습니다. 다른 후보를 선택하거나 다시 시도해 주세요.';});
     card.append(request);detail.append(card);return detail;
    }
    widget.addEventListener('gmp-load',()=>{
     if(ticket!==revision||!active())return;
     const places=widget.places||[];
     finish(!places.length?'일치하는 장소를 찾지 못했습니다. 사진 미확보는 시설 부족을 뜻하지 않습니다.':(places.length>1?'여러 장소 후보가 검색됐습니다. ':'')+'아래 사진은 Google 검색 후보의 미리보기입니다. 이름과 주소를 확인해 주세요. 자동으로 장소를 확정하지 않으며, 사진이 없으면 등록 사진 미확보입니다.');
     if(results.querySelector('.photo-detail, .photo-previews'))return;
     const previews=document.createElement('div');previews.className='photo-previews';
     places.slice(0,3).forEach((place,i)=>previews.append(photoCard(place,places.length>1?'장소 후보 '+(i+1)+' · 사진 미리보기':'검색된 장소 · 사진 미리보기','photo-preview')));
     if(places.length)results.append(previews);
    });
    ...
    widget.addEventListener('gmp-select',event=>{
     if(ticket!==revision||!active()||!event.place)return;
     results.querySelector('.photo-detail')?.remove();
     results.querySelector('.photo-previews')?.remove();
     const detail=photoCard(event.place,'선택한 장소의 사진','photo-detail');
   ...
   }catch(e){if(ticket===revision&&active()){status.textContent=e.message;button.disabled=false;}}
  };
  // Start only this school's search; do not block profile rendering or prefetch nearby places.
  button.onclick();
 }
