...
window.EducationLayers = (() => {
  ...
  function renderReport(row, body) {
    ...
    body.innerHTML = `<p class="edu-kicker">학교 생활권 · 공개자료 기반 정책 지원</p><h1>${e(row.학교명)}</h1><p>${e(row.학교급구분 || '초등학교')} · ${e(row.gu)} · ${e(row.소재지도로명주소 || '')}</p><button type="button" id="edu-ai-explain">이 학교의 분석 근거 설명</button>
      ...
      <h3>학교운동부 공개 운영 기록</h3><p>${e(schoolAthletics.coverage)}</p>${table(['게시일','운영 종목','공개 선수 인원','경비 원문'],(schoolAthletics.schools?.[getSchoolId(row)]||[]).map(a=>[a.posted_date,a.sport,a.athletes==null?a.athletes_raw:num(a.athletes,'명'),a.budget_raw]))}<details><summary>운동부 게시물 근거</summary>${(schoolAthletics.schools?.[getSchoolId(row)]||[]).map(a=>`<p><a href="${e(a.source_url)}" target="_blank" rel="noopener">${e(a.posted_date)} · ${e(a.school_name)} · ${e(a.sport)}</a></p>`).join('') || '연결 자료 미확보'}</details>
  ...
  function openStatistics(initialTab='statistics') {
    const dialog=ensureModal(), body=dialog.querySelector('.edu-body');
    ++reportRequest;
    ...
    const overview=document.createElement('section');overview.id='education-statistics-panel';
    while(body.firstChild)overview.appendChild(body.firstChild);
    overview.setAttribute('role','tabpanel');overview.setAttribute('aria-labelledby','education-statistics-tab');
    const insights=document.createElement('section');insights.id='education-insights-panel';
    insights.setAttribute('role','tabpanel');insights.setAttribute('aria-labelledby','education-insights-tab');
    const tabs=document.createElement('div');tabs.className='edu-analysis-tabs';tabs.setAttribute('role','tablist');tabs.setAttribute('aria-label','통계와 분석 결과');
    tabs.innerHTML='<button type="button" role="tab" id="education-statistics-tab" aria-controls="education-statistics-panel">현황·상관분석</button><button type="button" role="tab" id="education-insights-tab" aria-controls="education-insights-panel">분석 인사이트</button>';
    body.append(tabs,overview,insights);
    let insightsStarted=false;
    function selectTab(name){
      const active=name==='insights';overview.hidden=active;insights.hidden=!active;
      [...tabs.children].forEach((button,i)=>{const selected=i===(active?1:0);button.setAttribute('aria-selected',String(selected));button.tabIndex=selected?0:-1;});
      if(active&&!insightsStarted){
        insightsStarted=true;
        if(window.EducationNetworks) window.EducationNetworks.mountInsights(insights,document.getElementById('schoolLevelFilter')?.value,state.selectedGu);
        else insights.textContent='분석 화면을 불러오지 못했습니다. 페이지를 새로고침해 주세요.';
      }
    }
    [...tabs.children].forEach((button,i)=>{
      button.onclick=()=>selectTab(i?'insights':'statistics');
      button.onkeydown=event=>{if(['ArrowLeft','ArrowRight','Home','End'].includes(event.key)){event.preventDefault();const next=event.key==='Home'?0:event.key==='End'?1:1-i;selectTab(next?'insights':'statistics');tabs.children[next].focus();}};
    });
    const correlations=document.createElement('section');
    correlations.id='educationCorrelations';overview.appendChild(correlations);
    const legacy=document.createElement('button');legacy.type='button';legacy.textContent='기존 초등 통계 리포트 보기';
    legacy.onclick=()=>{dialog.close();openPreviewOverlay('statistics',true);};overview.appendChild(legacy);
