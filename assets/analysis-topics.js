...
window.AnalysisTopics=(()=>{
 ...
 const bar=document.createElement('details');bar.className='chat-save-bar';bar.innerHTML='<summary>대화 저장 · 새 대화</summary><p class="chat-save-status" role="status">이번 대화 0건 · 질문한 뒤 저장할 수 있습니다.</p><input class="chat-save-name" aria-label="대화 이름" maxlength="100" placeholder="대화 이름 (비우면 첫 질문으로 저장)"><button type="button" class="chat-save" disabled>대화 저장하기 ↓</button><button type="button" class="chat-delete danger" disabled>저장한 대화 삭제</button><button type="button" class="chat-new text-button">새 대화 시작</button>';
 $('messages').after(bar);const status=bar.querySelector('.chat-save-status'),saveButton=bar.querySelector('.chat-save');
 function refreshBar(){bar.querySelector('.chat-delete').disabled=!session.savedId;const n=session.entries.length;saveButton.disabled=!n;saveButton.textContent=session.savedId?'다시 저장하기 ↓':'대화 저장하기 ↓';status.textContent=n?`이번 대화 ${n}건 · ${session.savedId?(session.dirty?'저장 후 새 질문이 있습니다.':'저장됨 · ‘03 저장된 대화’ 탭에서 다시 볼 수 있습니다.'):'아직 저장하지 않았습니다. 대화가 끝나면 저장하세요.'}`:'이번 대화 0건 · 질문한 뒤 저장할 수 있습니다.';}
 ...
 async function save(){
  ...
  try{const name=(bar.querySelector('.chat-save-name').value.trim()||session.entries[0].question).slice(0,100),now=new Date().toISOString();
   ...
   await op('readwrite',s=>s.put(record));session.savedId=record.id;session.dirty=false;refreshBar();status.textContent=`‘${name}’ 저장 완료 · ${record.entries.length}건 · ‘03 저장된 대화’ 탭에서 확인하세요.`;await list();}
 ...
 async function removeSaved(id,name){
  if(!id||!confirm(`‘${name}’ 저장 대화를 삭제할까요?`))return;
  try{await op('readwrite',s=>s.delete(id));if(session.savedId===id){session.savedId=null;session.dirty=!!session.entries.length;refreshBar();status.textContent='저장한 대화를 삭제했습니다. 현재 화면의 대화는 남아 있습니다.';}await list();}
  catch(e){if(savedStatus)savedStatus.textContent='삭제 실패: '+e.message;status.textContent='삭제 실패: '+e.message;}
 }
 bar.querySelector('.chat-delete').onclick=()=>removeSaved(session.savedId,bar.querySelector('.chat-save-name').value||session.entries[0]?.question||'대화');
 function reset(){session.entries=[];session.savedId=null;session.dirty=false;bar.querySelector('.chat-save-name').value='';const evidence=$('evidence-panel');if(evidence)document.querySelector('.chat-workspace').append(evidence);window.ChatAgent?.history.splice(0);window.ChatMap?.dispose($('evidence-panel'));window.ChatMap?.dispose($('messages'));$('messages').replaceChildren();const panel=$('evidence-panel');if(panel)panel.innerHTML='<p class="eyebrow">근거 · 지도 · 표 · 출처</p><h3>답변의 근거가 여기에 표시됩니다.</h3><p class="muted">목록은 지도와 표로, 분석은 차트와 수치로 확인합니다. 출처를 누르면 제공기관 원문이 열립니다.</p>';refreshBar();}
 ...
 const safe=r=>window.ChatWorkspace?.safeResult?.(r)||r;
 const savedSummary=(entry,limit)=>{const result=safe(entry.result),text=result?.summary||entry.error||'결과 없음',shown=limit?text.slice(0,limit):text;return window.ChatWorkspace?.schoolText?window.ChatWorkspace.schoolText(shown,result||{}):esc(shown);};
 function restore(entry){document.querySelector('.workspace-nav [data-workspace="ask"]')?.click();const old=JSON.parse(JSON.stringify(safe(entry.result)));old.summary='저장 당시 기록입니다. 현재 지원안은 다시 계산해야 합니다. '+(old.summary||'');const panel=$('evidence-panel');if(panel)document.querySelector('.chat-workspace').append(panel);window.ChatWorkspace?.show(old,entry.question);const warning=document.createElement('p');warning.className='vs-history-warning';warning.textContent='저장 당시 관측·가정입니다. 현재 지원 판단으로 자동 복원하지 않습니다.';$('evidence-panel')?.prepend(warning);}
 ...
 function article(t){
  ...
  for(const [label,fn,cls] of [['이어서 질문하기',()=>{session.entries=t.entries.map(e=>JSON.parse(JSON.stringify(e)));session.savedId=legacy?null:t.id;session.dirty=false;const evidence=$('evidence-panel');if(evidence)document.querySelector('.chat-workspace').append(evidence);window.ChatAgent?.history.splice(0);window.ChatMap?.dispose($('evidence-panel'));window.ChatMap?.dispose($('messages'));$('messages').replaceChildren();for(const e of t.entries){const m=document.createElement('article');m.className='message';m.innerHTML=`<h3>${esc(e.question)}</h3><p>${savedSummary(e)}</p><p class="fine">저장된 대화에서 불러온 기록입니다.</p>`;if(e.result){const b=document.createElement('button');b.type='button';b.className='text-button';b.textContent='당시 근거 다시 보기 →';b.onclick=()=>restore(e);m.append(b);}$('messages').append(m);}refreshBar();document.querySelector('.workspace-nav [data-workspace="ask"]')?.click();},''],['JSON 저장 ↓',()=>download(t),''],['삭제',()=>removeSaved(t.id,t.name),'danger']]){const b=document.createElement('button');b.type='button';b.textContent=label;if(cls)b.className=cls;b.onclick=fn;actions.append(b);}
  const entries=item.querySelector('.saved-entries');
  for(const e of t.entries){const row=document.createElement('div');row.className='saved-entry';row.innerHTML=`<h4>${esc(e.question)}</h4><small>${esc(new Date(e.created).toLocaleString('ko'))} · ${esc(e.status)}</small><p>${savedSummary(e,600)}</p>`;if(e.result){const b=document.createElement('button');b.type='button';b.className='text-button';b.textContent='당시 근거 다시 보기 →';b.onclick=()=>restore(e);row.append(b);}const again=document.createElement('button');again.type='button';again.className='text-button';again.textContent='이 질문으로 새 분석 →';again.onclick=()=>{document.querySelector('.workspace-nav [data-workspace="ask"]')?.click();$('question').value=e.question;$('question').focus();};row.append(again);entries.append(row);}
  return item;
 }
 async function list(){
  if(!listHost)return;
  try{const rows=(await op('readonly',s=>s.getAll())).filter(t=>Array.isArray(t.entries)).sort((a,b)=>String(b.updated||b.created).localeCompare(String(a.updated||a.created)));listHost.replaceChildren();if(savedStatus)savedStatus.textContent=`저장된 대화 ${rows.length}건 · 이 브라우저에만 보관됩니다.`;if(!rows.length){listHost.innerHTML='<div class="saved-empty">저장된 대화가 없습니다. ‘02 자료에 묻기’에서 질문한 뒤 ‘대화 저장하기’를 누르세요.</div>';return;}for(const t of rows)listHost.append(article(t));if(savedStatus)savedStatus.textContent=`저장된 대화 ${rows.length}건 · 이 브라우저에만 보관됩니다.`;}
