...
const response=await page.request.post(base+'/api/chat',{timeout:120000,data:{scope:'all',level:'초등학교',question:'강화군 옹진군을 제외한 초등학교의 군구별 학급당 학생 수 평균을 비교하는 그래프를 보여줘. 학교 지원 순위를 만들지 말고 확보 학교 수와 단위를 포함해줘.',history:[]}});
...
await page.evaluate(cs=>{const panel=document.getElementById('evidence-panel');panel.innerHTML=cs.map(c=>window.ChatWorkspace.chart(c)).join('');panel.hidden=false;},charts);
assert((await page.locator('#evidence-panel .statistical-chart-dot').count())>0);await page.locator('#evidence-panel').screenshot({path:'outputs/visualization-20260916/question-chart.png'});
