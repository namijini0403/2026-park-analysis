...
(async()=>{
  ...
  const originalAwards=JSON.stringify(sportsSchool.awards);
  const compactSports=evidence.build(sportsSchool,'2025년 전국체육대회 메달 관측')[0].body;
  assert.doesNotMatch(compactSports,/"source_rows":/);
  assert.doesNotMatch(compactSports,/전국학생과학발명품경진대회|전국과학전람회/);
  assert(compactSports.includes(sportsSchool.awards.find(a=>a.participant_scope==='school_event_medal_observation'&&a.year===2025).source_url));
  assert.equal(JSON.stringify(sportsSchool.awards),originalAwards);
  assert(sportsSchool.awards.some(a=>a.source_rows?.length));
