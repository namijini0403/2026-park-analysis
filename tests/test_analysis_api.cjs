...
(async()=>{
 ...
 const comparison=await api.run({question:'연수구 남동구 공원 수 비교',options});
 assert.equal(comparison.status,'ok');assert.equal(comparison.plan.group_a,'연수구');assert.equal(comparison.plan.group_b,'남동구');
 const district=await api.run({question:'남동구 학원 수와 학생 수 관계',options});
 assert.equal(district.plan.gu,'남동구');
