...
const base=process.env.QA_BASE||'https://education-living-area-preview-production.up.railway.app',out='outputs/photo-auto-20260916';
...
const option=await page.locator('.school-photos option').nth(1).getAttribute('value');await page.locator('.school-photos select').selectOption(option);
...
fs.writeFileSync(out+'/browser-check.json',JSON.stringify({base,localAsset:process.env.QA_LOCAL_PHOTOS==='1',at:new Date(),passed:true,checks:['school automatic search and loaded photo','no implicit selection','manual facilities','school reselect','mobile width'],errors},null,2));console.log('PASS automatic school photo browser, manual facilities, loaded image, mobile');
