...
w.fetch=async(url,opts)=>{calls.push({url,body:opts?.body?JSON.parse(opts.body):null});if(url==='/api/chat')return {ok:true,json:async()=>answer};const u=new URL(url,'http://localhost');if(u.pathname==='/api/school-profile')return {ok:true,json:async()=>require('../api/_school_profile').profile(u.searchParams.get('id'))};return {ok:true,json:async()=>u.searchParams.get('id')?model.summary(u.searchParams.get('id')):{schools:model.list()}};};
const tick=(ms=30)=>new Promise(r=>setTimeout(r,ms));
(async()=>{
 for(const f of ['assets/chat-workspace.js','assets/source-evidence.js','assets/collection-assistant.js','assets/indicator-charts.js','assets/school-profile.js','assets/simple-app.js','assets/chat-agent.js'])w.eval(fs.readFileSync(path.join(root,f),'utf8'));
