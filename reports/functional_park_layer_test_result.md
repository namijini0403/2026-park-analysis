...
- 운영 루트 앱은 `data_processed/school_priority_with_functional_park_layer.csv`의 기능성 공원 컬럼과 보행부담 컬럼을 읽는다.
- 표지 제목은 `반경 너머,` / `도달 가능성으로` 의미 단위 줄바꿈으로 고정했고 큰 제목 크기를 낮췄다.
- 학교 상세 리포트의 상단 최근접 공원·녹지 카드에는 요약 수치와 한 줄 해석만 남겼다.
- 최근접 공식 공원명, 공원 유형, 면적, 기능등급, 활동 가능 공원 거리, 보행부담 등급, 간선도로 횡단, 대형 교차로 여부는 `가장 가까운 공원의 이용 조건` 카드로 분리했다.
