...
function build(row,question){
  ...
  if(/미래|예측|수요|인구|전망|학생수|원아|prophet|xgboost/i.test(q)) parts.push(chunk(row,'demand','현재·미래 수요',`현재 학생·원아 ${fmt(row.current_students)}명. 학교 주변 해당 연령 거주인구(현재 배분 추정, 재학생 수·미래 예측 아님): ${JSON.stringify(row.current_age_demand)}. 학교 이력·지원 예측: ${JSON.stringify(row.enrollment)}. 지역 통계 연결(2025년 경계 기준, 개편 후 구 전체 수치가 아님): ${JSON.stringify(row.statistical_region_2025)}. 지역 연령 예측: ${JSON.stringify(row.regional)}. 학교 재학생·구군 연령 인구·후보지 시나리오는 서로 다른 모집단이다. 최근 연속 3개년 미만 이력인 기관은 학교별 장기 예측을 만들지 않으며 구·군 연령 인구 예측과 구분한다. 유치원 원아수는 만3·4·5세와 혼합 원아의 합이며 특수학급 수치는 별도 공시로 확인한다.`));
