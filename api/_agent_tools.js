...
function query_schools(ctx,p){
 ...
 const cols=[...new Set(['name','gu',...(sortBy?[sortBy]:[]),...(p.columns||[]).filter(c=>!unset(c)&&c!=='name')])];cols.forEach(c=>{col(ctx,c);usedCols.add(c);});
