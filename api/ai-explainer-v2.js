...
function detectTopic(payload) {
  ...
  // "권고 안정성", 바로 "우선" 대신 "우선 검토안"(카드의 기본 행동 표기)만 매칭해 case 라벨
  // ("우선 검토 대상")과도 구분한다. "시나리오"도 단독으로 두면 "가중치는 왜 시나리오 장치라고
  // 부르나요"(decision 소관) · "정책 시나리오 트랙이 뭐야"(metrics 소관) 같은 질문을 가로채므로,
  // 카드의 12조합 시나리오를 가리키는 복합 표현으로만 매칭한다.
  if (/행동 ?카드|정책 ?카드|우선 ?검토안|조건부 ?대안|전환 ?조건|기관 ?역할|권고 ?안정성|카드 ?시나리오|시나리오 ?조합|시나리오 ?축|시나리오 ?조건/.test(question)) return "policy";
...
const DOMAIN_SIGNAL_PATTERN = new RegExp(
  [
    "공원|녹지|놀이터|놀이시설|학교|초등|유치원|중등|고등|학생|아동|어린이|학원|교습|예체능|수능|성취|학력|성적|운동부|선수 인원|수상|실적|메달|체육대회|발명|과학전람회|진학|졸업|취업|공시|장학|체력|동아리|방과후|paps",
    ...
    "재개발|대단지|아파트|도로|횡단|안전|사고|내부 통행|주거 구역|투과",
