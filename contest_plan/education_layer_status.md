...
## 2026-09-09 수상 질문 근거 길이 최적화

과학전람회·발명·체육대회 수집 범위 설명을 각각 보존하고 질문한 대회의 설명만 전달하도록 바꿨다. 복수 대회 또는 전체 실적 질문은 해당 범위를 함께 유지한다. 공식 결과 원문에 다시 접근하는 데 필요한 학교·연도·종목·세부종목·등급·공식 URL·식별 근거는 유지하며, 감사용 source_rows 표·행 위치 배열만 모델 입력 예시에서 제외한다. 원자료·서버 학교 근거에는 원래 행 위치가 남아 있다. 학교별 보고서의 상세 자료는 변경하지 않는다.

85개 학교·주제 조합을 같은 질문으로 비교했다. 근거 body 문자 수 중앙값은 체육대회 33개 조합에서 2,130→1,579(25.9% 감소), 발명 32개에서 2,059→1,658(19.5%), 과학전람회 20개에서 1,555→1,052(32.3%)였다. 과학전람회 조합에는 교육청 보도자료 연결도 포함한다. 이는 문자 수 비교이며 실제 토큰·요금 실측치가 아니다. 특정 대회 질문에 다른 대회 설명이 포함되지 않는지, 공식 출처가 유지되는지, 원본 수상 배열이 변경되지 않는지 및 기존 AI 검사를 통과했다.
