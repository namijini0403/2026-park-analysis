...
- 배포 ID: `3fce34e7-b66f-4f65-8e59-464f14dd7b71` — Railway `SUCCESS` 확인.
- 소스 커밋: `61cc8f3`, 대화창 차트 폭 보완 `0fba9b5`. 협업 브랜치에 push 완료.
- 새 스냅샷 `outputs/hitl-deploy-1789528045281`, 1,504개 manifest 항목 검증. 마지막 차트 CSS 보완은 이 새 스냅샷의 원본·정적 복사본에 동일하게 복사한 뒤 전체 manifest를 다시 검증했다. 중간 소스가 바뀐 첫 스냅샷은 무결성 검사에서 실패하여 배포하지 않았다.
- 공개 index·통계 JS/CSS·공통 차트 JS/CSS·복구 디자인 파일 총 8개가 로컬 바이트와 일치한다. 기록: `outputs/visualization-20260916/deployment-check.json`.
- 운영 통계 API: 공원 영역 지표 8개·전체 카탈로그 43개, 일반/강화옹진 확보 및 전체 분모 확인.
- 실제 운영 Chromium 데스크톱/390px 모바일: 검토 안내, 히스토그램, 군구→학교 필터, 영역간 산점도, 도서 분리, 결측 보류, 가로 넘침 없음, 페이지 JS 오류 0건.
- 실제 운영 챗봇 질문: 강화·옹진 제외 초등학교 군구별 학급당 학생 수 평균. 서버의 집계 도구 호출 결과를 실제 `ChatWorkspace.show`로 그려 제목·단위·값과 대화창 내부 가로 넘침 없음을 확인했다. 사용자 대화 저장·외부 메시지 발송은 하지 않았다.
- `outputs/visualization-20260916/browser-check.json`, `question-chart-check.json`, PNG에 공개 검증 기록.
