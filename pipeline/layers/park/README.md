...
| **`scripts/accessibility/update_manual_verified_nearest_parks.py`** | 수동검증 80개교 결과 | `school_nearest_park.csv` 갱신 | **⚠️ 재실행 금지, 봉인값 보호 — 최우선 확인 대상** |
| **`scripts/accessibility/recalc_nearest_park_public_only.py`** | `parks.csv`, `sealed_nearest_park_dist.json` | `school_nearest_park.csv` 재계산 | **⚠️ 재실행 금지, 봉인값 보호**(코드 내 봉인값을 덮어쓰지 않는 로직 포함되어 있으나 원본 재실행은 금지) |
