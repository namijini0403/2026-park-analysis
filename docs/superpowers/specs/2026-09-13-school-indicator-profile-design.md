...
2026-09-19 사용자 요청으로 레이더는 **바깥쪽일수록 긍정 방향**으로 통일한다. 원자료 백분위와 상세 막대는 유지하고 레이더 반지름만 `up`: 백분위, `down`: 100 − 백분위로 표시한다. `neutral`은 방향을 단정하지 않고 레이더에서 제외하며 영역별 상세에 남긴다. 지표를 합산하거나 투자 순위를 만들지 않는다.
...
- 반지름 = 긍정 방향 상대 위치 (바깥쪽 = 긍정). `up`은 원자료 백분위, `down`은 100 − 원자료 백분위다. ↑·↓는 원자료의 유리한 방향을 표시한다. 중립 지표는 레이더 밖에서 별도로 안내한다.
- 토글 2개: `군·구 기준` / `인천 전체 기준`
- 미확보 축은 0이 아니라 **끊어진 축**(점선, 값 없음)으로 그린다.
- 차트 아래 고정 문구: `모양 비교용이며 순위·종합점수가 아닙니다. 축마다 단위와 성격이 다릅니다.`

**규칙 예외 기록**: AGENTS.md는 서로 다른 성격의 지표를 교환 가능한 숫자로 비교하지 말 것을 요구한다. 레이더는 이 규칙과 부분적으로 충돌하며, 사용자가 충돌을 확인한 뒤 2026-09-13에 명시적으로 선택했다. 2026-09-19에는 긍정·부정 방향 통일을 요청했다. 완화 장치(영역 합산 없음, 원자료 백분위 유지, 중립 지표 별도 안내, 끊어진 축, 방향 변환과 모양 비교 고지)를 함께 구현한다. AGENTS.md에도 변경된 예외를 기록한다.
